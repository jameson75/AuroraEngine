...
namespace CipherPark.Aurora.Core.Effects
{
    public class EdgeDraw : SurfaceEffect
    {
        ...
        public EdgeDraw(IGameApp game)
            : base(game)
        {
            _shaderByteCode = LoadVertexShader("Assets\\Shaders\\edgedraw-vs.cso", out _vertexShader);
            LoadGeometryShader("Assets\\Shaders\\edgedraw-gs.cso", out _geometryShader);
            LoadPixelShader("Assets\\Shaders\\edgedraw-ps.cso", out _pixelShader);
