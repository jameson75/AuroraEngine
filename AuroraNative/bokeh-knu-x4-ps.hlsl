...
float4 main( PSIN input ) : SV_TARGET
{
	...
	color += lastpass.Sample(passSampler, float2(tex.x, tex.y-(rcpres.y)*scale )) * 56;
	color += lastpass.Sample(passSampler, float2(tex.x, tex.y+(rcpres.y)*scale )) * 56;
 
	color += lastpass.Sample(passSampler, float2(tex.x, tex.y-(rcpres.y*2)*scale )) * 28;
	color += lastpass.Sample(passSampler, float2(tex.x, tex.y+(rcpres.y*2)*scale )) * 28;
 
	color += lastpass.Sample(passSampler, float2(tex.x, tex.y-(rcpres.y*3)*scale )) * 8;
	color += lastpass.Sample(passSampler, float2(tex.x, tex.y+(rcpres.y*3)*scale )) * 8;
 
	color += lastpass.Sample(passSampler, float2(tex.x, tex.y-(rcpres.y*4)*scale )) * 1;
	color += lastpass.Sample(passSampler, float2(tex.x, tex.y+(rcpres.y*4)*scale )) * 1;
